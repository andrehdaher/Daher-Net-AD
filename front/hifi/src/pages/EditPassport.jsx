...
const EditPassport = () => {
  ...
  const navigate = useNavigate(); // لتوجيه المستخدم بعد التعديل
  ...
  useEffect(() => {
    const fetchPassport = async () => {
      try {
        const response = await axios.get(`https://daher-net-ad-43.onrender.com/api/passports/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(`https://daher-net-ad-43.onrender.com/api/passports/${id}`, formData, {
        headers: {
      ...
      setTimeout(() => {
        navigate("/passports", { replace: true }); // التوجيه بعد التعديل
      }, 1000); // إضافة تأخير 1 ثانية لضمان نجاح العملية
    } catch (error) {
